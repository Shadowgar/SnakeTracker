...
Planning only. Do not execute until the owner explicitly approves this document. The immutable architecture baseline is Git commit `bb3ab39` (`docs: approve SnakeTracker architecture baseline`). ADR-0028 is active.
...
4. Approve the proposed initial SQLite candidates: FULL durability, 5-second busy timeout, 1,000-page auto-checkpoint, 256 MiB journal limit, subject to Pi qualification without weakening durability.
5. Approve a 90% line / 85% branch coverage gate for Phase 1 code.
6. Approve loopback-only Nginx in Phase 1 and complete exclusion of cloudflared/remote access until the RD gate.
7. Approve a minimal baseline Alembic revision containing no identity, event, household, animal, job, or product tables.
